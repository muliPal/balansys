...
export abstract class peer extends mypanel {
	...
	async show_selection(clicked?: cell): Promise<void> {
		...
		if (!cell2)
			throw new mutall_error(
				`Unable to find cell ['${row}', '${col}'] of this peer`
			);
		//
	...
	get_modified_driver_sql(): string | undefined {
		...
		const where: string = transcription.consumer_pk
			? `where consumer.consumer = ${transcription.consumer_pk}`
			: "";
		//
...
export class transcription extends panel.group {
	...
	static Initial_display: { consumer: number; image: number } = {
		consumer: 2,
		image: 0,
	};
	//
	...
	zoom_in(dir: boolean): void {
		...
		let image = <HTMLImageElement>(
			this.document.querySelector(".selected>img")
		);
		//
...
export class consumer extends mypanel {
	...
	async show_selection(): Promise<void> {
		...
		if (!cell)
			throw new mutall_error(
				`No cell is found at coordinate [${row} ${col}] of consumer`
			);
		//
...
class purchase extends mypanel {
		...
		const options: table_options = {
			...
			ticks: [
				...
				[
					"unit",
					{},
					[undefined, "product", "unit"],
					[undefined, "purchase", "unit"],
				],
			],
		};
		//
	...
	async show_selection(): Promise<void> {
		...
		if (!cell)
			throw new mutall_error(
				`No cell is found at coordinate [${row}, ${col}]`
			);
		//
		//Select the cell
		cell.select();
...
export class supplier extends mypanel {
	...
	public plan: plan = [
		...
		[
			this.get_leftie(),
			new panel.reviewer(this, 0, false, "open", {
				onchange: (cell: cell) => this.onclick_from_reviewer(cell),
			}),
			this,
			//
