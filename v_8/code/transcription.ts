...
import { view,view_options, label, mutall_error, basic_value } from "../../../schema/v/code/schema.js";
//
//This file is being developed as part of the mutall library
import {panel, container, panel_options, group, reviewer, creator} from "./panel.js";
...
export abstract class peer extends panel{
    //
    ...
    constructor(
        ...
        parent: group, 
        selection_cname: string
    ){
        super(sql, row_index_cname, options, parent, selection_cname);
    }
    //
    ...
    // by a scrlling into view
    show_selection(cell?:grid):void{
        //
        //Do not repaint this peer if the given cell comes from this zone 
        if (cell?.parent === this) return;
        //
        //Get the primary key of the image to show
        const pk:string|undefined = transcription.image_pk;
        //
        ...
        //the list view
        cell2.td.scrollIntoView({block:'start'})
    }
    ...
    async modify_driver():Promise<void>{
        ...
        this.driver_source!.sql=sql;
    }
}

//
//Transcription is the page that helps us to digitize images of physical receipts
export class transcription extends group{
    //
    public consumer:panel;
    public image_panel:peer;
    public file:peer;
    public receipt:peer;
    public purchase:panel;
    //
    //This helps us to manage the peer panels 
    public image_group:group;
    //
    ...
    static display:{consumer:number, image:number} = {consumer:2, image:0};
    ...
    constructor(
        ...
        this.image_group = new group(this, {}, []);
        //
    ...
    //Override the show method so that  we show the selections as well (in addition to what?)
    async show():Promise<void>{
        //
        //Show the members of this transcription group and their selections
        for(const member of this.members){
            //
            //Show the member
            await member.show();
            //
            //Highlight the current selection and it scroll into view
            member.show_selection();
        }
    }
    //
...
class file extends peer{
    ...
    constructor(parent:group){
        ...
        //    
        super(sql, 'image.image',  options, parent, 'path');
    }   
}
...
class image extends peer{
    ...
    public plan:plan = [
        ...
        [new homozone(null), new creator(this, 1)],
    ...
    constructor(parent:group){
        ...
        //    
        super(sql, 'image.image',  options, parent,'image');
    }   
}

//The panel that shows the owners/consumers of thses receipts
export class consumer extends panel{
    //
    ...
    public plan:plan = [
        //
        //The headers
        [new homozone(), this.get_header(), new homozone(null)],
        //
        ...
        [new reviewer(this,0, false, 'open',{onchange:(cell:cell)=>this.onclick_from_reviewer(cell)}), this, new reviewer(this,0, true, 'write')],
        //
        //The record create functionality. The only reviewer on the right is unchecked 
        // will write the record if checked
        [new homozone(null), this.creator, new reviewer(this.creator, 0, false, 'write')],
    ];
    //
    constructor(parent:view){
        ...
        //    
        super(sql, 'consumer.consumer',  options, parent, 'consumer.name');
    ...
    show_selection(): void {
        ...
        //
        //Get the indexed grid from the indexed cells of this homozone
        const cell:grid = this.cells_indexed![row][col]; 
        //
...
class receipt extends peer{
    ...
    public plan:plan = [
        ...
        [
            this.get_leftie(), 
            new reviewer(this, 0, false, 'open',{onchange:(cell:cell)=>this.onclick_from_reviewer(cell)}), 
            this, 
            //
            //Do not refresh the panel after writing
            new reviewer(this, 0, true, 'write',{refresh_after_write:false})
        ],
    ];
    //
    ...
    constructor(parent:group){
        ...
        //    
        super(sql, 'image.image',  options, parent, 'receipt.ref');
...
class purchase extends panel{
    ...
    plan:plan = [
        //
        //The headers
        [new homozone(null), this.get_header(), new homozone(null)],
        //
        //There is a review on both sides of the body with a row orientation
        [new reviewer(this, 0, false, 'open'), this, new reviewer(this, 0, true, 'write')],
        //
        //The creator
        [new homozone(null), this.creator, new reviewer(this.creator, 0, false, 'write')],
    ...
    constructor(parent:transcription){
        ...
        //    
        super(sql, 'purchase.purchase',  options, parent, 'ref');
    }
    ...
    async modify_driver():Promise<void>{
        ...
        //
        //Get the current driver source
        const {row_index} = <{type:'sql', sql:string, row_index:string}>this.driver_source;
        //
        ...
        this.driver_source = {type:'sql', sql, row_index};
    }
    ...
    show_selection(): void {
        ...
        // 
        //Get the last row
        const row:string = ticks[ticks.length-1].toString();
        //
