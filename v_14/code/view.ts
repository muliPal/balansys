...
export class view extends EventTarget{
   ...
   public get_attribute_value(element:HTMLElement, name:string):string{
    ...
    myget<i>(x:i|undefined, name?:string):i{
        //
    ...
    */
    public get_value(id:string):string|null|Error{
        ...
        //the above example
        const env:HTMLElement = this.get_envelop_element(id);
        //
        ...
        const io_type:io_type = this.get_io_type_from_envelop(env);
        //
    ...
    //files
    get_files(id:string):FileList|null|Error{
        //
        //Get the identified enveloping element, e.g., the label element in the 
        //the above example
        const env:HTMLElement = this.get_envelop_element(id);
        ...
        //check boxes, if there is nothing checked, the raw value is null. For 
        //simple input, the null is a zero-length string
        const file:FileList|null = this.get_files_using_envelop(env);
        //
        //Determine whether the value is required or not;
        const is_required:boolean = Boolean(env.dataset.required);
        //
        //If an input is required and it is empty, return the an error...
        if (is_required && file===null) return new Error(`Input '${id}' is required`);
        //
    ...
    get_files_using_envelop(env:HTMLElement):FileList|null{
        //
        //Use the envelop to search for an input element of type file
        const inputs:NodeList = env.querySelectorAll('input[type="file"]');
        //
        //Its an error if none is found
        if (inputs.length===0) throw new mutall_error(`No file input element is found under current envelop element`, env);
        //
        //It islao an errror if the search result is ambiguous
        if (inputs.length>1) throw new mutall_error(`There is more than 1 file input element under curremt envelop`, env);
        //
    ...
    get_envelop_element(id:string):HTMLElement{
        //
        //Let element be the envelop
        let element:HTMLElement|null;
        //
        //Try the data-field route first
        if (element = this.document.querySelector(`*[data-field="${id}"]`)) return element;
        //
        //Try the normal id route
         if (element = this.document.getElementById(id)) return element;
        //
        //Try name route
        const elements:Array<HTMLElement> = Array.from(this.document.getElementsByName(id));
        if (elements.length>0) {
            //
            const data_field:HTMLElement|null = elements[0].closest('*[data-field], *[id]');
            //
             if (data_field) return data_field;
        }    
        //
        //Element not found
    ...
    private get_io_type_from_envelop(env:HTMLElement):io_type{
        //
    ...
    */
    private get_radio_value(env:HTMLElement):string|null{
        ...
        const name:string|undefined = env.dataset.field; 
        if (!name) throw new mutall_error(`The envlop enclosing radio buttons must have a data field named the same as the buttons`, env);
        //
        ...
        const radios:NodeListOf<HTMLElement>= env.querySelectorAll(css);
        //
        ...
        const checkeds:NodeListOf<HTMLElement>= env.querySelectorAll(`${css}:checked`);
        //
    ...
    private get_text_value(env:HTMLElement):string|null{
        ...
        if (elements.length>1) throw new mutall_error(`Only 1 value element is expected. ${elements.length} were found`);
        //
