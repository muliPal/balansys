...
export abstract class peer extends mypanel{
    ...
    async onclick(cell:cell, evt:MouseEvent):Promise<void>{
        ...
        tv.members_indexed!.get('image_group').show_selection(cell);
        //
        //Re-show the supplier
        await tv.members_indexed!.get('supplier').show();
        //
        //Re-show the purchases
        await tv.members_indexed!.get('purchase').show();
    }

    //
    //Shows the selection of this peer, by highlighting the image followed
    // by a scrolling into view. The given cell is the relvant one for this 
    // homozone
    show_selection(clicked?:cell):void{
        //
        ...
        if (pk===undefined) throw new mutall_error('Selection not found!');
        //
        ...
        const cell2:cell = this.cells_indexed![row][col];
        //
...
export class transcription extends panel.group{
    //
    //A transcription has the consumer, image_group, supplier and purchase panels
    //These are containers indexed by the constructor name
    public members_indexed?:mymap<string, panel.container>;
    //
    ...
    // from any where, i.e., without following the view has-a hierarchy. What do 
    // we lose by so doing? 
    ...
    constructor(
        ...
        const Purchase = new purchase(this);
        //
        //Create the image grpup
        //
        const Supplier = new supplier(this);

        //Define an empty image group
        const Image_group = new image_group(this)
        //
        //Create the 3 members of the transcription group
        const containers:Array<panel.container> = [
            Consumer,
            Image_group,
            Supplier,
            Purchase,
        ...
        //
        //Map the contaners to the transcript map entries
        const t_entries:Array<[string, panel.container]> = containers.map(container=>{
            //
            //Contaimers will be indexed by their constructor name
            const name:string = container.constructor.name; 
            //
            //Compile teh entry
            const entry:[string, panel.container] = [name, container];
            //
            return entry;
        });
        //
        //Save the panel collection
        this.members_indexed = new mymap('Panel', t_entries);
    }
...
export class consumer extends mypanel{
    ...
    async onclick(cell:cell, evt?:MouseEvent):Promise<void>{
        ...
        //transcription show
        await trans.members_indexed!.get('image_group').show();
        //
        //Show the transcription purchases
        await trans.members_indexed!.get('purchase').show();
    }
}
...
export class supplier extends mypanel{
    ...
    constructor(parent:view){
        ...
        const options: table_options={
            ...
            ticks:[
                ...
                ['supplier.name', {labels:[[undefined, 'business', 'name']]}]
            ],
        }; 
        //
    ...
    //Show the only record in this panel
    public show_selection(clicked?: cell): void {
        //
        const cell:cell|undefined = this.cells_relative?.[0]?.[0];
        //
        if (cell) cell.select();
    }
...
export class image_group extends panel.group{
    ...
    constructor(parent:transcription){
        ...
        const Image_panel = new image(this);
        const Receipt =    new receipt(this);
        //
        //Collect the panel peers
        const peers:Array<mypanel> = [File, Image_panel, Receipt];
        //
